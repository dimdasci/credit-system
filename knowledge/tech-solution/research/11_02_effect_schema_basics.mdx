...
<Aside type="caution" title="Property Addition Only">
  ...
  property, it cannot replace or override an existing one.
</Aside>
...
<Aside type="caution" title="Empty Structs Allow Any Data">
  Using `Schema.Struct({})` results in a TypeScript type `{}`, which behaves
  similarly to `unknown`. This means that any data will be considered valid, as
  there are no defined constraints.
</Aside>
